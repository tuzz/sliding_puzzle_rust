...
fn main() {
    ...
    queue.push_back((0, Direction::Left));

    while !queue.is_empty() {
        depth += 1;
        width = queue.len();
        percent += queue.len() as f64 / (max as f64 / 2.0) * 100.0;
        hardest = queue.front().unwrap().0.clone();

        println!("{:8.1}%  |  depth: {}, width: {}", percent, depth, width);

        while let Some((decimal, previous)) = queue.pop_back() {
            ...
            for direction in puzzle.moves() {
                ...
                if seen.insert(next_decimal as usize) {
                    next_queue.push_back((next_decimal, direction));
                }
            }
        }

        queue = next_queue;
        next_queue = VecDeque::<(u64, Direction)>::with_capacity(queue.len());
